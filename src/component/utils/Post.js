...
import { getDatabase, onValue, get, ref, remove, set, update, push } from 'firebase/database';
...
const createComment = async (postId) => {
    const commentText = comment.current.value; // Use comment
    ...
    try {
        const db = getDatabase();
        const commentsListRef = ref(db, `comments`); // Reference to the 'comments' collection

        // Use push() to create a new unique child node and write data to it directly
        const newCommentRef = push(commentsListRef); // Get a new unique reference
        const newComment = await set(newCommentRef, { // Set data to that new reference
 ...
 useEffect(()=>{
  const db = getDatabase();
   const refComments =  ref(db,'comments')
   onValue(refComments,(snapshot)=>{
     const comments = snapshot.val();
     const tempStore = [];
     Object.entries(comments).forEach(([id,comment])=>{
       tempStore.push({id:id,...comment})
     })

    setComments(tempStore)
  ...
  return (
      ...
      <div   className='all_comment_list h-[280px] relative overflow-hidden rounded-lg bg-white'>
            ...
            comments.filter(comment=> comment.post_id == post.id).map((comment) =>
               ...
               <div className='cmnt__rply'>
                   ...
                   <div className='flex items-center gap-3 p-5 border-b border-gray-200'>
                     <img className='mobile:w-6 mobile:h-6 tablet:w-10 tablet:h-10 rounded-full border-2 object-cover border-brd' src={user?.profile_picture} alt='puser_image'/>
                     <div>
                       <Link className='mobile:text-xs tablet:text-base text-black' to={'/'}>
                         {user?.username || <Skeleton/>}
                       </Link>
                       <p className='mobile:text-xs tablet:text-base text-gray-600'>{comment.comment_text}</p>
                     </div>
                   </div>
                   {/* Reply */}
        ...
        <div className='absolute w-full left-0 h-[60px] bottom-0 bg-gray-400 flex items-center gap-3 p-5'>
            <img className='w-10 h-10 rounded-full border-2 object-cover border-brd' src={user?.profile_picture} alt='puser_image'/>
            <input ref={comment} type="text" placeholder='Write a comment...' className='h-10 border text-semi-black font-nunito font-normal mobile:text-sm tablet:text-base border-bdr w-full outline-none focus:outline-none  border-b-2 rounded px-2 focus:border-0'/>
            <button onClick={()=>createComment(post.id)} className='bg-blue-500 flex mobile:text-sm tablet:text-base items-center gap-3 text-white px-4 py-2 rounded'><FiSend className='text-base text-white cursor-pointer'/>Post</button>
        </div>
      </div>
      }
      </div>
